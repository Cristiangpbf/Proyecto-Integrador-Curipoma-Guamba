    ...
    if(user_type==='admin'){
        return(
            <Menu.SubMenu icon={ <UserOutlined /> } title={ currentUser && currentUser.name  } >
                ...
                <Link to={ Routes.EMPLOYEES }><Menu.Item key='setting:3'><UsergroupAddOutlined /> Empleados</Menu.Item></Link>
                <Link to={ Routes.PRODUCTS }><Menu.Item key='setting:4'><BarsOutlined /> Productos</Menu.Item></Link>
